        ...
        <div className="p-6">
          ...
          <ul className="space-y-3">
            {report.feedback.map((msg, idx) => {
              const isPositive = msg.startsWith('Great') || msg.startsWith('Success');
              return (
                <li key={idx} className="flex items-start gap-3 text-gray-700">
                  <span className={`mt-1 flex-shrink-0 w-2 h-2 rounded-full ${isPositive ? 'bg-green-500' : 'bg-red-500'}`} />
                  <span className={isPositive ? 'font-medium' : ''}>{msg}</span>
                </li>
              );
            })}
          </ul>
