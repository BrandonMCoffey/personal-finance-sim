import type { Account, Income, TransferRule, Goal, WinConditions, Expense, Card, Loan, Retirement } from '../store/financeStore';
...
export function evaluatePlan(
  ...
  // Check Goals
  goals.forEach(goal => {
    const configuredGoalDeadline = winConditions.goalsFundedWithinMonths?.goalId === goal.id
  ...
  expenses.forEach(expense => {
    ...
    if (violation) {
      ...
      feedback.push(`Minimum Funding Warning: "${expense.name}" falls below its $${expense.minValue!.toFixed(2)} minimum in Month ${violation.month}, receiving only $${actual.toFixed(2)}. (-${VARIABLE_EXPENSE_MIN_PENALTY} pts)`);
    }
  });
  ...
  if (winConditions.maxCashBalance !== undefined) {
    ...
    const sweepForecast = generateForecast(accounts, incomes, transferRules, goals, expenses, cards, loans, retirements, 1);
